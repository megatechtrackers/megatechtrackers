"""Codec 8E decoder implementation."""
...
class Codec8E:
    ...
    def _decode_avl_data(self) -> AvlData:
        ...
        properties_count = self._reader.read_int16()
        io_properties = self._decode_io_properties()
        
        io_element = IoElement.create(event_id, properties_count, io_properties)
    ...
    def _decode_io_properties(self) -> List[IoProperty]:
        ...
        io_count_int8 = self._reader.read_int16()
        ...
        io_count_int16 = self._reader.read_int16()
        ...
        io_count_int32 = self._reader.read_int16()
        ...
        io_count_int64 = self._reader.read_int16()
        for i in range(io_count_int64):
            property_id = self._reader.read_int16()
            value = self._reader.read_int64()
            result.append(IoProperty.create(property_id, value))
        
        # Variable-length properties (X)
        io_count_x = self._reader.read_int16()
        for i in range(io_count_x):
            property_id = self._reader.read_int16()
            element_length = self._reader.read_int16()
            value = self._reader.read_bytes(element_length)
            result.append(IoProperty.create_array(property_id, value))
