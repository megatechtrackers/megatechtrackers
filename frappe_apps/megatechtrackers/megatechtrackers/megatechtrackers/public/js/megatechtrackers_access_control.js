...
// Auto-populates forms and reports dropdowns
...
function setup_report_assignment_table(frm) {
	...
	// Handle Link field display refresh when report is selected
	frappe.ui.form.on('AC Grafana Report Assignment', {
		ac_report: function(frm, cdt, cdn) {
			const row = locals[cdt][cdn];
			if (row.ac_report) {
				// Force refresh the Link field to show title
				frappe.model.set_value(cdt, cdn, 'ac_report', row.ac_report);
