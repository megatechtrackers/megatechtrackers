...
Aligns with consumer/parser nodes: RotatingFileHandler, config-driven level/rotation, suppress noisy libs.
"""
import json
import logging
import os
...
def setup_logging_from_config() -> None:
    ...
    try:
        ...
        log_file = log_config.get("log_file", os.path.join("logs", "metric_engine.log"))
        ...
        logger.info(
            "Logging configured: file=%s, level=%s, json=%s",
            log_file,
            log_level_str,
            enable_json_logging,
        )
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler("logs/metric_engine.log"),
                logging.StreamHandler(sys.stdout),
            ],
        )
        logging.getLogger(__name__).warning(
            "Failed to configure logging from config: %s, using defaults", e
        )
